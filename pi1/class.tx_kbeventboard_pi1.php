...
class tx_kbeventboard_pi1 extends tslib_pibase {
	...
	function showStandard() {
		...
		if($this->selectedcategory > 0){
			...
			if($this->show_time_mode == 1){ // all events
				...
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev LEFT JOIN tx_kbeventboard_locations lo ON ev.location = lo.uid LEFT JOIN tx_kbeventboard_category ca ON ev.category = ca.uid','ev.deleted=0 
										  ...
										  AND (FIND_IN_SET('.$this->selectedcategory.',ev.category))','','ev.datebegin '. $this->ascDesc . '');
			...
			if($this->show_time_mode == 1){ // all events
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev, tx_kbeventboard_locations lo','ev.deleted=0 
								      AND ev.hidden=0 '.$this->eventFolderSQLPart.' 
								      AND (ev.fe_group = 0 OR ev.fe_group IN ('.implode(',',$this->aktUserGroup).')) 
								      AND ev.location = lo.uid ','','ev.datebegin '. $this->ascDesc . '');								  
			}else if($this->show_time_mode == 2){ // only old events
		...
		if($this->selectedcategory > 0){
			...
			if($this->show_time_mode == 1){ // all events
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev LEFT JOIN tx_kbeventboard_locations lo ON ev.location = lo.uid LEFT JOIN tx_kbeventboard_category ca ON ev.category = ca.uid','ev.deleted=0 
									      ...
									      AND (ev.fe_group = 0 OR ev.fe_group IN ('.implode(',',$this->aktUserGroup).')) 
									      AND ev.location = lo.uid 
										  AND (FIND_IN_SET('.$this->selectedcategory.',ev.category))','','ev.datebegin '. $this->ascDesc . '',''.$this->pos.','.$this->step.'');
				...
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev LEFT JOIN tx_kbeventboard_locations lo ON ev.location = lo.uid LEFT JOIN tx_kbeventboard_category ca ON ev.category = ca.uid','ev.deleted=0 
									      ...
									      AND ev.location = lo.uid 
										  AND (FIND_IN_SET('.$this->selectedcategory.',ev.category))
										  AND (ev.dateend <= '.$this->yesterday.')','','ev.datebegin '. $this->ascDesc . '',''.$this->pos.','.$this->step.'');
				...
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev LEFT JOIN tx_kbeventboard_locations lo ON ev.location = lo.uid LEFT JOIN tx_kbeventboard_category ca ON ev.category = ca.uid','ev.deleted=0 
								      ...
								      AND ev.location = lo.uid 
									  AND (FIND_IN_SET('.$this->selectedcategory.',ev.category))
								      AND ((ev.datebegin + 86400 + ev.delaytime) > '.$this->today.' OR ev.dateend > '.$this->yesterday.')','','ev.datebegin '. $this->ascDesc . '',''.$this->pos.','.$this->step.'');
			}
			...
			if($this->show_time_mode == 1){ // all events
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev, tx_kbeventboard_locations lo','ev.deleted=0 
								      AND ev.hidden=0 '.$this->eventFolderSQLPart.' 
								      AND (ev.fe_group = 0 OR ev.fe_group IN ('.implode(',',$this->aktUserGroup).')) 
								      AND ev.location = lo.uid ','','ev.datebegin '. $this->ascDesc . '',''.$this->pos.','.$this->step.'');
			}else if($this->show_time_mode == 2){ // only old events
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*,ev.uid','tx_kbeventboard_events ev, tx_kbeventboard_locations lo','ev.deleted=0 
								      AND ev.hidden=0 '.$this->eventFolderSQLPart.' 
								      AND (ev.fe_group = 0 OR ev.fe_group IN ('.implode(',',$this->aktUserGroup).')) 
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			// more Link:
			if(intval($this->showMorelink) == 1){
				$marker["###MORE_LINK###"] = '<a href="'. htmlspecialchars($this->pi_linkTP_keepPIvars_url(Array('evt' => $row['uid']),1,0,$this->morelinkId)). '" class="kb_eventboard_morelink" >'.htmlspecialchars($this->pi_getLL('morelink')).'</a>';
				// set Link title:
	...
	function showSingleView() {
		...
		if($this->eventid > 0){
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				// more Link:
				if(intval($this->showMorelink) == 1){
					$marker["###MORE_LINK###"] = '<a class="kb_eventboard_morelink" href="'. htmlspecialchars($this->pi_linkTP_keepPIvars_url(Array('evt' => $row['uid']),1,0,$this->morelinkId)). '" class="kb_eventboard_morelink" >'.htmlspecialchars($this->pi_getLL('morelink')).'</a>';
				}else{
					$marker["###MORE_LINK###"] = '';
				}
...
//###########################################################################	
	...
	function showContextCol() {
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			// more Link:
			if(intval($this->showMorelink) == 1){
				$marker["###MORE_LINK###"] = '<a class="kb_eventbox_morelink" href="'. htmlspecialchars($this->pi_linkTP_keepPIvars_url(Array('evt' => $row['uid']),1,0,$this->morelinkId)). '" class="kb_eventboard_morelink" >'.htmlspecialchars($this->pi_getLL('morelink')).'</a>';
				// set Link title:
				if(intval($this->showTitleLink) == 1){
